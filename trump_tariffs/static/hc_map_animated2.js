document.addEventListener("DOMContentLoaded", function() {
    (async () => {
        ...
        const fakePieData = {
            'USA': [{ name: 'Mobile', y: 60 }, { name: 'Desktop', y: 40 }],
            'IND': [{ name: 'Mobile', y: 80 }, { name: 'Desktop', y: 20 }],
            'BRA': [{ name: 'Mobile', y: 70 }, { name: 'Desktop', y: 30 }],
            'default': [{ name: 'Mobile', y: 50 }, { name: 'Desktop', y: 50 }]
        };
        ...
        let isPlaying = false;
        let selectedYear = 1;
        let yearInterval = null; // to hold the interval for play functionality
    
        function generateBubbleData(year) {
            return Object.entries(coordinatesMap).map(([code, coords]) => {
                const match = data.find(row => row.code === code && row.year === year);
                ...
                return {
                    ...
                    lon: coords.lon,
                    value: value,
                    //color: '#FF5733',       // Explicit bubble color
                    color: getColor(value, code), // conditional bubble color
                    fillOpacity: 0.9,
                    lineColor: '#222',
                    lineWidth: 1,
                };
        ...
        function generateBubbleData(year) {
            return Object.entries(coordinatesMap).map(([code, coords]) => {
                ...
                return {
                    ...
                    lon: coords.lon,
                    value: value,
                    color: getColor(value, code), // Choropleth-matched bubble color
                    fillOpacity: 0.9,
                    lineColor: '#222',
                    lineWidth: 1
                };
        ...
        function updateMapForYear(year) {
            ...
            const newDataLayer1 = economiescolor.map(region => {
                ...
                // Filter data by code and the selected year
                const match = data.find(row => row.code === code && row.year === year);
                const value = match ? parseFloat(match.value) : null;
            ...
            map.series[6].setData(generateBubbleData(year), true); // Update bubbles
        ...
        document.getElementById('playPauseButton').addEventListener('click', function() {
            if (isPlaying) {
                ...
                yearInterval = setInterval(() => {
                    selectedYear++; // Use selectedYear directly
                    if (selectedYear > 3) {
                        selectedYear = 1; // Loop back to the start
                    }
                    updateMapForYear(selectedYear); // Update the map with the new year
        ...
        document.getElementById('yearSlider').addEventListener('input', function() {
            selectedYear = parseInt(this.value); // Set the selected year from the slider
            updateMapForYear(selectedYear); // Update the map with the selected year
        });

        //generate bubbleData to start with
        const bubbleData = generateBubbleData(selectedYear);

        // function to show pie chart on click
        function showPieChart(code) {
            const data = fakePieData[code] || fakePieData['default'];
            document.getElementById("pieOverlay").style.display = "block";
        ...
        function hidePieChart() {
            document.getElementById("pieOverlay").style.display = "none";
        }
        ...
        const map = Highcharts.mapChart('container', {
            ...
            series: [
...

                {
                    ...
                    zMax: maxValue,
                    tooltip: {
                        pointFormat: '<b>{point.name}</b><br>Value: {point.z:.2f}'
                    },
                    plotOptions: {
                        series: {
                            point: {
        ...
        // Initialize map with the selected year (on page load or reset)
