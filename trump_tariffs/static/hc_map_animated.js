import { showPieChart, hidePieChart } from './overlay.js';
...
document.addEventListener("DOMContentLoaded", function() {
    (async () => {
        ...
        function updateMapFortariff_structure(tariff_structure) {
            ...
            // If overlay is open, update bar charts too
            const overlay = document.getElementById("pieOverlay");
            if (overlay && overlay.classList.contains("visible") && activeCountryCode) {
            showPieChart(activeCountryCode, tariff_structure);
            }
        ...
        document.getElementById('tariff_structureSlider').addEventListener('input', function() {
            ...
            const overlay = document.getElementById("pieOverlay");
            if (overlay && overlay.classList.contains("visible") && activeCountryCode) {
              showPieChart(activeCountryCode, selectedtariff_structure);
            }
        });
        ...
        document.getElementById("closePieButton").addEventListener("click", hidePieChart);
            ...
            series: [
...

                {
                    ...
                    point: {
                        events: {
                            click: function () {
                            console.log("Bubble clicked:", this);
                            const code = this.options.code;
                            activeCountryCode = code;
                            showPieChart(code, selectedtariff_structure); 
                            }
                        }
                    },                      
                },
